      ...
      <form className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          <div className="flex gap-4">
            <input onChange={(e)=>setFiles(e.target.files)} className='w-full border border-gray-300 p-3 rounded' type="file" id='images' accept='images/*' multiple />
          ...
          <p className='text-sm text-red-700'>{imageUploadError &&imageUploadError}</p>
          {
            formData.imageUrls.length>0 && formData.imageUrls.map((url,index)=>(
              <div key={url} className="flex justify-between border items-center">
                ...
                <button type='button' onClick={()=>handleRemoveImage(index)} className=' text-red-700 p-3 hover:opacity-75 uppercase'>Delete</button>
              </div>
            ))
          }
